...
/**
Represents mime part of the message.
**/
class mime
{
public:
    ...
    **/
    struct content_type_t
    {
        ...
        **/
        ...
        /**
        Initializes media type to none, subtype and charset to empty strings.
        **/
        content_type_t();

        /**
        Initializes content type with the given media type and subtype.
    ...
    // TODO: what is a differrence between ascii and seven bit?
    /**
    Content transfer encodings.
    
    @todo Missing binary type?
    **/
    enum class content_transfer_encoding_t {NONE, ASCII, BIT_7, BIT_8, BASE_64, QUOTED_PRINTABLE};
    ...
    **/
    ...
    @throw mime_error  Non multipart message with boundary.
    @throw *           `format_header`, `format_content`.
    **/
    void format(std::string& mime_str, bool dot_escape = true) const;

    /**
    Parses mime part from a string.
    ...
    **/
    ...
    /**
    Parses a line of mime part.

    The method is used to parse message string line by line, as received over the protocols. For each line a crl is appended. If the line is
    ...
    **/
    ...
    /**
    Sets content transfer encoding.
    ...
    **/
    ...
    /**
    Sets boundary of the mime part.
    ...
    **/
    ...
    /**
    Gets the boundary of the mime part.
    ...
    **/
    ...
    /**
    Sets the content given as string.
...
protected:
    ...
    **/
    ...
    /**
    Formats content by using the codec.

    @param dot_escape Flag if leading dots in lines should be escaped.
    @return           Formatted content.
    @throw *          `bit7::encode`, `bit8::encode`, `base64::encode`, `quoted_printable::encode`.
    **/
    std::string format_content(bool dot_escape) const;

    /**
    Parses header by going through header lines and calling `parse_header_line`.
    
    @throw * `parse_header_line`.
    **/
    void parse_header();

    /**
    Parses the content by using the appropriate codec.

    @throw * `bit7::decode`, `bit8::decode`, `base64::decode`, `quoted_printable::decode`.
    **/
    void parse_content();
    ...
    @throw *           `parse_header_name_value`, `parse_content_type`, `parse_content_transfer_encoding`, `parse_content_disposition`.
    **/
    virtual void parse_header_line(const std::string& header_line);
    
    /**
    Parses header for the name and value.
    
    @param header_line  Header to parse.
    @param header_name  Header name parsed.
    @param header_value Header value parsed.
    @throw mime_error   Bad header name.
    @throw mime_error   Bad header line.
    @throw mime_error   Header name or value empty.
    **/
    void parse_header_name_value(const std::string& header_line, std::string& header_name, std::string& header_value);

    /**
    Parses content type and its attributes.
    ...
    @throw mime_error       Bad content type value.
    @throw *                `parse_header_value_attributes`, `mime_type_as_enum`.
    **/
    static void parse_content_type(const std::string& content_type_hdr, media_type_t& media_type, std::string& media_subtype,
        std::map<std::string, std::string>& attributes);

    /**
    Parses content transfer encoding value and attributes.
    ...
    @throw mime_error            Bad content transfer encoding.
    @throw *                     `parse_header_value_attributes`.
    **/
    static void parse_content_transfer_encoding(const std::string& transfer_encoding_hdr, content_transfer_encoding_t& encoding,
        std::map<std::string, std::string>& attributes);
    
    /**
    Parses content disposition value and attributes.
    ...
    @throw mime_error       Bad content disposition.
    @throw *                `parse_header_value_attributes`.
    **/
    static void parse_content_disposition(const std::string& content_disp_hdr, content_disposition_t& disposition,
        std::map<std::string, std::string>& attributes);
    
    /**
    Parses value and attributes of the so called content headers.
    ...
    @throw mime_error Bad header value.
    @throw mime_error Bad attribute name.
    @throw mime_error Bad attribute value.
    @todo             Allowed characters are more strict than required?
    **/
    static void parse_header_value_attributes(const std::string& header, std::string& value, std::map<std::string, std::string>& attributes);

    /**
    Creates random boundary for the mime part.
    ...
    **/
    static std::string mime_type_as_str(media_type_t media_type_val);
    ...
    **/
    static media_type_t mime_type_as_enum(const std::string& media_type_val);

    /**
    Removes trailing empty lines from the body.
    **/
    void strip_body();
    ...
    **/
    ...
    @todo Ahould it contain filename of the attachment?
    **/
    std::string _name;
    ...
    **/
    ...
    /**
    Keeps status of mime parsing: is parsing of the mime part started or stopped.
    **/
