...
const string mime::HEADER_NAME_ALPHABET{"-"};
const string mime::CONTENT_ATTR_ALPHABET{"!#$%&*+-.^_`|~"};
const string mime::CONTENT_HEADER_VALUE_ALPHABET{"/-"};


mime::mime() : _version("1.0"), _content_type(media_type_t::NONE, ""), _encoding(content_transfer_encoding_t::NONE),
    _disposition(content_disposition_t::NONE), _line_policy(codec::line_len_policy_t::RECOMMENDED), _parsing_header(true),
    _mime_status(mime_parsing_status_t::NONE)
{
}
...
codec::line_len_policy_t mime::line_policy()
{
    return _line_policy;
}
...
string mime::format_header() const
{
    ...
    switch (_disposition)
    {
        case content_disposition_t::ATTACHMENT:
            header += CONTENT_DISPOSITION_HEADER + COLON + CONTENT_DISPOSITION_ATTACHMENT + "; filename=\"" + _name + "\"\r\n";
            break;

        case content_disposition_t::INLINE:
            header += CONTENT_DISPOSITION_HEADER + COLON + CONTENT_DISPOSITION_INLINE + "; filename=\"" + _name + "\"\r\n";
            break;
...
// TODO: line policy should be applied
void mime::parse_header()
{
    ...
    {
        if (isspace(hdr[0]))
            line += codec::SPACE_CHAR + trim_copy(hdr);
        else
        {
            if (!line.empty())
            {
...
void mime::parse_content()
{
    ...
    switch (_encoding)
    {
        ...
        case content_transfer_encoding_t::ASCII:
        case content_transfer_encoding_t::BIT_8:
        {
            bit8 b8(_line_policy);
...
void mime::parse_header_line(const string& header_line)
{
    ...
    else if (iequals(header_name, CONTENT_DISPOSITION_HEADER))
    {
        ...
        map<string, string>::iterator filename_it = attributes.find("filename");
        if (filename_it != attributes.end())
            _name = filename_it->second;
    }
}


void mime::parse_header_name_value(const string& header_line, string& header_name, string& header_value)
{
    ...
    while (colon_pos++ < header_line.length())
    {
        ...
        if (is_header_name && !isalpha(header_line[colon_pos]) && HEADER_NAME_ALPHABET.find(header_line[colon_pos]) == string::npos)
    ...
    // TODO: header value can be empty?
    if (header_name.empty() /*|| header_value.empty()*/)
        throw mime_error("Header name or value empty.");
}


void mime::parse_content_type(const string& content_type_hdr, media_type_t& media_type, string& media_subtype, map<string, string>& attributes)
{
    string header_value;
...
void mime::parse_content_transfer_encoding(const string& transfer_encoding_hdr, content_transfer_encoding_t& encoding, map<string, string>& attributes)
{
    string header_value;
...
void mime::parse_content_disposition(const string& content_disp_hdr, content_disposition_t& disposition, map<string, string>& attributes)
{
    string header_value;
...
Implementation goes by using state machine. Diagram is shown in graphviz dot language:
    ...
    digraph header_value_attributes
    {
        ...
        qattrvaluebegin -> qattrvaluebegin [label="token"];
...
*/
void mime::parse_header_value_attributes(const string& header, string& header_value, map<string, string>& attributes)
{
...
string mime::mime_type_as_str(media_type_t media_type_val)
{
    string mime_str;
...
mime::media_type_t mime::mime_type_as_enum(const string& media_type_val)
{
    media_type_t media_type;
