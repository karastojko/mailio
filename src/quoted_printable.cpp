...
vector<string> quoted_printable::encode(const string& text) const
{
    ...
    for (auto ch = text.begin(); ch != text.end(); ch++)
    {
        ...
        {
            // add soft break
            if (line_len >= int(_line_policy) - 3)
            {
                line += EQUAL_CHAR;
        ...
        else if (*ch == SPACE_CHAR)
        {
            ...
            if (line_len == int(_line_policy) - 4)
            {
                line += SPACE_CHAR;
                line += EQUAL_CHAR;
                enc_text.push_back(line);
            ...
            else if (line_len == int(_line_policy) - 3)
            {
                line += EQUAL_CHAR;
                enc_text.push_back(line);
                line.clear();
        ...
        {
            if (ch + 1 == text.end() || (ch + 1 != text.end() && *(ch + 1) != LF))
        ...
        else
        {
            ...
            if (line_len >= int(_line_policy) - 5)
            {
                line += EQUAL_CHAR;
                enc_text.push_back(line);
                line.clear();
...
string quoted_printable::decode(const vector<string>& text) const
{
    ...
    {
        if (line.length() > int(_line_policy) - 2)
