...
vector<string> quoted_printable::encode(const string& text) const
{
    ...
    for (auto ch = text.begin(); ch != text.end(); ch++)
    {
        ...
        {
            if (line_len >= string::size_type(_line_policy) - 2)
            {
                if (_q_codec_mode)
                {
                    enc_text.push_back(line);
        ...
        else
        {
            ...
            if (line_len >= string::size_type(_line_policy) - 5 && !_q_codec_mode)
            {
                line += EQUAL_CHAR;
                enc_text.push_back(line);
                line.clear();
            ...
            else if (line_len >= string::size_type(_line_policy) - 2 && _q_codec_mode)
            {
                enc_text.push_back(line);
                line.clear();
                line += EQUAL_CHAR;
