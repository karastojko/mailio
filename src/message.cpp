...
message::message() : mime(), _header_codec(header_codec_t::QUOTED_PRINTABLE)
{
...
void message::header_codec(header_codec_t hdr_codec)
{
    _header_codec = hdr_codec;
}


message::header_codec_t message::header_codec() const
{
...
string message::format_header() const
{
    ...
    // TODO: move formatting subject to a separate method
    // TODO: formatting subject by using `codec::line_len_policy_t::SUBJECT` is not correct
    header += SUBJECT_HEADER + COLON;
    if (codec::is_utf8_string(_subject))
    {
        q_codec qc(codec::line_len_policy_t::SUBJECT, _header_codec);
        vector<string> hdr = qc.encode(_subject);
        header += hdr.at(0) + codec::CRLF;
        if (hdr.size() > 1)
            for (auto h = hdr.begin() + 1; h != hdr.end(); h++)
                header += " " + *h + codec::CRLF;
    }
    // TODO: fix for bug #65 goes here
    else
        header += _subject + codec::CRLF;
    header += codec::CRLF;
...
// TODO: move q codec detection to a separate method
string message::parse_subject(const string& subject)
{
    string dec_subject, encoded_part;
    bool q_encoded = false;
    ...
    {
        if (*it == codec::QUESTION_MARK_CHAR)
            ++question_mark_counter;

        if (*it == codec::EQUAL_CHAR && it + 1 != subject.end() && *(it + 1) == codec::QUESTION_MARK_CHAR && !q_encoded)
            q_encoded = true;
        else if (*it == codec::QUESTION_MARK_CHAR && it + 1 != subject.end() && *(it + 1) == codec::EQUAL_CHAR && question_mark_counter == QUESTION_MARKS_NO)
        {
            q_encoded = false;
            question_mark_counter = 0;
            dec_subject += qc.decode(encoded_part);
            encoded_part.clear();
            it++;
        }
        else if (q_encoded == true)
            encoded_part.append(1, *it);
        else
            dec_subject.append(1, *it);
    }

    if (q_encoded && question_mark_counter < QUESTION_MARKS_NO)
...
string message::parse_address_name(const string& address_name)
{
    q_codec qc(_line_policy, _header_codec);
