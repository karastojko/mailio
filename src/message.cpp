...
string message::parse_subject(const string& subject)
{
    ...
    {
        if (*it == codec::EQUAL_CHAR && it + 1 != subject.end() && *(it + 1) == codec::QUESTION_MARK_CHAR && !encoded)
            encoded = true;
        else if (*it == codec::QUESTION_MARK_CHAR && it + 1 != subject.end() && *(it + 1) == codec::EQUAL_CHAR)
        {
            encoded = false;
