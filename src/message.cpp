...
namespace mailio {
    ...
    message::message()
            : mime(), _date_time(second_clock::universal_time(), time_zone_ptr(new posix_time_zone("00:00"))) {
    }
    ...
    string message::format_header() const {
        ...
        header += _sender.address.empty() ? "" : SENDER_HEADER + HEADER_SEPARATOR_STR + sender_to_string() +
                                                 codec::END_OF_LINE;
        header += _reply_address.name.empty() ? "" : REPLY_TO_HEADER + HEADER_SEPARATOR_STR +
                                                     reply_address_to_string() + codec::END_OF_LINE;
        header += TO_HEADER + HEADER_SEPARATOR_STR + recipients_to_string() + codec::END_OF_LINE;
        header += _cc_recipients.empty() ? "" : CC_HEADER + HEADER_SEPARATOR_STR + cc_recipients_to_string() +
                                                codec::END_OF_LINE;
        header += _bcc_recipients.empty() ? "" : BCC_HEADER + HEADER_SEPARATOR_STR + bcc_recipients_to_string() +
                                                 codec::END_OF_LINE;
        header += _disposition_notification.empty() ? "" : DISPOSITION_NOTIFICATION_HEADER + HEADER_SEPARATOR_STR +
                                                           format_address(_disposition_notification.name,
                                                                          _disposition_notification.address) +
                                                           codec::END_OF_LINE;
...
*/
    void message::parse_header_line(const string &header_line) {
        ...
        if (iequals(header_name, FROM_HEADER)) {
            ...
            //comment these code, because parse massage-id causes error
            //auto ids = parse_many_ids(header_value);
            //if (!ids.empty())
            //    _message_id = ids[0];

            _message_id = header_value;
            ...
            /*like message_id, don't parse it, and ignore references*/
    ...
    string message::format_address_list(const mailboxes &mailbox_list) const {
        ...
        for (auto mg = mailbox_list.groups.begin(); mg != mailbox_list.groups.end(); mg++) {
            ...
            mailbox_str += mg != mailbox_list.groups.end() - 1 ? string(1, MAILGROUP_SEPARATOR) + codec::END_OF_LINE +
                                                                 NEW_LINE_INDENT : string(1, MAILGROUP_SEPARATOR);
        }
...
*/
    local_date_time message::parse_date(const string &date_str) const {
        try {
            ...
            if (regex_match(date_str, m, r)) {
                ...
                string dttz = m[1].str() + " " + (m[2].str()[1] == ' ' ? "0" : "") + m[2].str() + " " +
                              m[3].str().substr(0, 12) + ":" + m[3].str().substr(12);
    ...
    string message::parse_address_name(const string &address_name) const {
        ...
        bool is_q_encoded = address_name.size() >= Q_CODEC_SEPARATORS_NO && address_name.at(0) == codec::EQUAL_CHAR &&
                            ...
                            address_name.at(addr_len - 1) == codec::EQUAL_CHAR &&
