...
string message::format_address(const string& name, const string& address) const
{
   ...
   if (regex_match(address, m, DTEXT_REGEX))
    ...
    return (snd_name.empty() ? addr : snd_name + " " + addr);
}
...
Meanings of the labels:
  ...
  * `addr`: address only    
*/
vector<message::mail_info> message::parse_address_list(const string& address)
{
    ...
    string tmp;
    for (auto ch = address.begin(); ch != address.end(); ch++)
    {
        switch (state)
        {
            case state_t::BEGIN:
           ...
           case state_t::ADDR:
               if (isalpha(*ch) || isdigit(*ch) || DTEXT.find(*ch) != string::npos)
                   tmp += *ch;
               else if (isspace(*ch))
                   ;
