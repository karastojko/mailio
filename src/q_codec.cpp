...
q_codec::q_codec(codec_method_t codec_method) : codec(line_len_policy_t::RECOMMENDED), _codec_method(codec_method)
{
}
...
vector<string> q_codec::encode(const string& text) const
{
    ...
    if (_codec_method == codec_method_t::BASE64)
    {
        ...
        base64 b64(codec::line_len_policy_t::HEADER);
        text_c = b64.encode(text);
    }
    else
    {
        codec_flag = QP_CODEC_STR;
        quoted_printable qp(codec::line_len_policy_t::HEADER);
...
string q_codec::decode(const string& text) const
{
    ...
    if (to_upper_copy(charset) != "UTF-8")
        throw codec_error("Bad encoding.");
    ...
    string dec_text;
    if (method == BASE64_CODEC_STR)
    {
        base64 b64(_line_policy);
        dec_text = b64.decode(text_c);
    }
    else if (method == QP_CODEC_STR)
    {
